class JobApi {
    ...
    async getJobList(saved = false) {
        ...
        try {
            let response = await fetch(`http://localhost:8000/api/v1/jobs${query}`, option);
    ...
    async postNewResume(resume) {
        try {
            ...
            let response = await fetch('http://localhost:8000/api/v1/resume', options);
            return await response.json();
        } catch(err) {
            console.log(err);
        }
    ...
    async getResumeById(id) {
        try {
            let response = await fetch(`http://localhost:8000/api/v1/resume/${id}`, {
                method: 'GET'
            });
            return await response.json();
        } catch(err) {
    ...
    async saveJob(id) {
        try {
            ...
            let response = await fetch(`http://localhost:8000/api/v1/jobs/`, options);
            return await response.json();
        } catch(err) {
            console.log(err);
        }
